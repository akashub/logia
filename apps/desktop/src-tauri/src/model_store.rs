...
pub(crate) fn selected(folder: &Path) -> &'static Model {
    let mut id = String::new();
    if let Ok(file) = std::fs::File::open(folder.join("selected-model")) {
        let _ = file.take(128).read_to_string(&mut id);
    }
    catalog::offerable().find(|model| model.id == id.trim()).unwrap_or_else(catalog::default_model)
}
pub(crate) fn select(folder: &Path, model: &Model) -> Result<(), String> {
...
pub(crate) fn remove(folder: &Path, model: &Model) -> Result<(), String> {
...
pub(crate) fn verify_model(path: &Path, model: &Model) -> Result<(), String> {
...
pub(crate) struct Mutation(Arc<AtomicBool>);
impl Drop for Mutation { fn drop(&mut self) { self.0.store(false, Ordering::Release); } }
pub(crate) fn begin_mutation(active: bool, gate: &Arc<AtomicBool>) -> Result<Mutation, String> {
